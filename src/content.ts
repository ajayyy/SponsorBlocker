...
async function whitelistCheck(videoID: string) {
    ...
    const getChannelInfo = (videoID: string) => {
        ...
        return {
            id: channelNameTag.getAttribute("href").match(/\/channel\/(UC[a-zA-Z0-9_-]{22})/)[1],
            name: channelNameTag.textContent.trim()
        };
    };

    try {
        let tempChannelInfo;
        await utils.wait(() => !!(tempChannelInfo = getChannelInfo(videoID))?.id, 6000, 20);

        channelIDInfo = {
            status: ChannelIDStatus.Found,
            id: tempChannelInfo.id,
            name: tempChannelInfo.name
        }
    } catch (e) {
        channelIDInfo = {
    ...
    channelWhitelisted = channelSpecificSettings != undefined &&
